...
const categoryGradients: Record<string, string> = {
  Formal: "bg-gradient-to-br from-slate-300/60 via-slate-200/60 to-slate-50/90",
  "Business Casual": "bg-gradient-to-br from-sky-300/60 via-sky-200/60 to-white/90",
  Professional: "bg-gradient-to-br from-indigo-300/60 via-indigo-200/60 to-white/90",
  Creative: "bg-gradient-to-br from-rose-300/55 via-orange-200/60 to-white/90",
  Seasonal: "bg-gradient-to-br from-emerald-300/60 via-emerald-200/55 to-white/90",
}
...
export default function OutfitSelection({ outfit, setOutfit }: OutfitSelectionProps) {
  ...
  const selectedOutfit = outfits.find((o) => o.text === outfit)
  const selectedIndex = useMemo(
    () => outfits.findIndex((o) => o.text === outfit),
    [outfit]
  )
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") {
        e.preventDefault()
        newIndex = Math.min(focusedIndex + 1, outfits.length - 1)
      } else if (e.key === "ArrowLeft") {
        e.preventDefault()
        newIndex = Math.max(focusedIndex - 1, 0)
      } else if (e.key === "Enter" || e.key === " ") {
        e.preventDefault()
        setOutfit(outfits[focusedIndex].text)
        return
      } else if (e.key === "Home") {
        e.preventDefault()
        newIndex = 0
        ...
        e.preventDefault()
        newIndex = outfits.length - 1
      }

      if (newIndex !== focusedIndex) {
        setFocusedIndex(newIndex)
        const buttons = scrollContainerRef.current?.querySelectorAll("button[data-outfit-option]")
            ...
            {selectedOutfit && (
              <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
                ...
                <span className="hidden text-[10px] text-muted-foreground md:text-xs sm:inline-flex">
                  {selectedIndex + 1} of {outfits.length}
                </span>
              </div>
            )}
          </div>
